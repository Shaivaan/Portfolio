...
const fetchUserData = async (user_id,handleData) => {
    try {
        ...
        const projectsQuery = query(collection(firebaseFirestore, 'projects'), where('user_id', '==', user_id));
        const projectsQuerySnapshot = await getDocs(projectsQuery);
        const projectsData = projectsQuerySnapshot.docs.map(doc => doc.data());
        const workExpQuery = query(collection(firebaseFirestore, 'workExp'), where('user_id', '==', user_id));
